...
if (process.env.REACT_APP_NETWORK === 'mainnet') {
  ...
  url = 'https://api.studio.thegraph.com/query/33920/dai-goerli/v0.0.17';
} else {
  VEST_DAI_IDS = 0;
  VEST_MKR_TREASURY_IDS = 0;
  url = 'https://api.studio.thegraph.com/query/33920/dai-goerli-test/v0.0.64';
}
...
class App extends Component {
  ...
  all = async (blockNumber) => {
    ...
    const getBlockIntervalPeriods = (networkString) => {
      let blockInterval = 45500 /* ≈ 7 day */,
        periods = 52; /* 12 months */
      // get current date
      ...
      if (currentTimestamp < startTimestamp + 60 * 60 * 24 * 365) {
        periods = Math.max((((currentTimestamp - startTimestamp) / (60 * 60 * 24 * 7)) | 0) - 4, 0);
        if (periods < 2) {
          blockInterval = 0;
        }
      }
    ...
    if (process.env.REACT_APP_NETWORK === 'mainnet') {
      ...
      var [[block, res], ethSupply, ethPriceNxt, ethPriceMedian, fauPriceNxt, fauPriceMedian, historicalDebt] =
        structuredResult;
    }
