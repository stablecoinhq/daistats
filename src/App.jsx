...
if (process.env.REACT_APP_NETWORK === 'mainnet') {
  add = require('./addresses-mainnet.json');
...
const build = (address, name) => {
  return new ethers.Contract(address, require(`./abi/${name}.json`), provider ? provider : eth);
};
...
if (process.env.REACT_APP_NETWORK === 'mainnet') {
  ...
  subgraphUrl = 'https://api.studio.thegraph.com/query/33920/dai-goerli/v0.0.17';
} else {
  VEST_DAI_IDS = 0;
  VEST_MKR_TREASURY_IDS = 0;
  subgraphUrl = 'https://api.studio.thegraph.com/query/33920/dai-goerli-test/v0.0.67';
}
...
class App extends Component {
  ...
  all = async (blockNumber) => {
    ...
    if (process.env.REACT_APP_NETWORK === 'mainnet') {
      p1 = multi.callStatic.aggregate(
        [
          ...
          [
            add.MCD_JOIN_DIRECT_AAVEV2_DAI_INCENTIVE,
            aaveIncentive.interface.encodeFunctionData('getRewardsBalance', [[add.ADAI], add.MCD_JOIN_DIRECT_AAVEV2_DAI]),
          ],
          ...
          .concat(
            this.getIlkCall(univ2wbtcethAIlkBytes, 'UNIV2WBTCETH_A', univ2wbtceth, add.UNIV2WBTCETH, add.PIP_UNIV2WBTCETH),
          )
          .concat(
            this.getIlkCall(univ2usdcethAIlkBytes, 'UNIV2USDCETH_A', univ2usdceth, add.UNIV2USDCETH, add.PIP_UNIV2USDCETH),
          )
          .concat(
            this.getIlkCall(univ2daiusdcAIlkBytes, 'UNIV2DAIUSDC_A', univ2daiusdc, add.UNIV2DAIUSDC, add.PIP_UNIV2DAIUSDC),
          )
          .concat(
            this.getIlkCall(univ2linkethAIlkBytes, 'UNIV2LINKETH_A', univ2linketh, add.UNIV2LINKETH, add.PIP_UNIV2LINKETH),
          )
          .concat(this.getIlkCall(univ2uniethAIlkBytes, 'UNIV2UNIETH_A', univ2unieth, add.UNIV2UNIETH, add.PIP_UNIV2UNIETH))
          .concat(
            this.getIlkCall(univ2wbtcdaiAIlkBytes, 'UNIV2WBTCDAI_A', univ2wbtcdai, add.UNIV2WBTCDAI, add.PIP_UNIV2WBTCDAI),
          )
          .concat(
            this.getIlkCall(univ2aaveethAIlkBytes, 'UNIV2AAVEETH_A', univ2aaveeth, add.UNIV2AAVEETH, add.PIP_UNIV2AAVEETH),
          )
          ...
          .concat(
            this.getIlkCall(
              guniv3daiusdc1AIlkBytes,
              'GUNIV3DAIUSDC1_A',
              guniv3daiusdc1,
              add.GUNIV3DAIUSDC1,
              add.PIP_GUNIV3DAIUSDC1,
            ),
          )
          .concat(
            this.getIlkCall(
              guniv3daiusdc2AIlkBytes,
              'GUNIV3DAIUSDC2_A',
              guniv3daiusdc2,
              add.GUNIV3DAIUSDC2,
              add.PIP_GUNIV3DAIUSDC2,
            ),
          )
          ...
          .concat(
            this.getIlkCall(crvv1ethstethAIlkBytes, 'CRVV1ETHSTETH_A', crvv1ethsteth, add.CRVV1ETHSTETH, add.PIP_CRVV1ETHSTETH),
          ),
      ...
      p1 = multi.callStatic.aggregate(aggregated, { blockTag: blockNumber });
    }
    let promises;
    if (process.env.REACT_APP_NETWORK === 'mainnet') {
      promises = [
    ...
    if (process.env.REACT_APP_NETWORK === 'mainnet') {
      var [
        ...
        maticPriceMedian,
        univ2daiethPriceNxt,
        univ2wbtcethPriceNxt,
        univ2usdcethPriceNxt,
        univ2daiusdcPriceNxt,
        univ2linkethPriceNxt,
        univ2uniethPriceNxt,
        univ2wbtcdaiPriceNxt,
        univ2aaveethPriceNxt,
        guniv3daiusdc1PriceNxt,
        guniv3daiusdc2PriceNxt,
        wstethPriceNxt,
        wstethPriceMedian,
      ...
      var [[block, res], ethSupply, ethPriceNxt, ethPriceMedian, fauPriceNxt, fauPriceMedian, historicalDebt, allVaults] =
        structuredResult;
    }
    ...
    let d3mAdaiTargetSupply,
      d3mAdaiBar,
      d3mAdaiAvailableLiquidity,
      d3mAdaiDaiDebt,
      d3mAdaiTotalSupplyVariable,
      d3mAdaiTotalSupplyFixed;
    if (process.env.REACT_APP_NETWORK === 'mainnet') {
  ...
  getHistoricalDebt = async ({ blockInterval, periods }) => {
    try {
      ...
      if (latestBlock) {
        const _numberOfPoints = periods ?? latestBlock / blockInterval;
        return [];
      }
    } catch (err) {
        ...
        <Router basename="/">
          ...
          <div className="notification is-primary has-text-centered">
            ...
            <div className="buttons is-centered">
              ...
              <button className="button is-small is-rounded" onClick={() => this.props.toggle('es')}>
                Español
              </button>
              <button className="button is-small is-rounded" onClick={() => this.props.toggle('fr')}>
                Français
              </button>
              <button className="button is-small is-rounded" onClick={() => this.props.toggle('it')}>
                Italiano
              </button>
              <button className="button is-small is-rounded" onClick={() => this.props.toggle('de')}>
                Deutsch
              </button>
              <button className="button is-small is-rounded" onClick={() => this.props.toggle('id')}>
                Bahasa Indonesia
              </button>
              <button className="button is-small is-rounded" onClick={() => this.props.toggle('zh-TW')}>
                繁體中文
              </button>
              <button className="button is-small is-rounded" onClick={() => this.props.toggle('jp')}>
                日本語
              </button>
              <button className="button is-small is-rounded" onClick={() => this.props.toggle('ru')}>
                Русский
              </button>
              <button className="button is-small is-rounded" onClick={() => this.props.toggle('ga')}>
                Gaeilge
              </button>
              <button className="button is-small is-rounded" onClick={() => this.props.toggle('tr')}>
                Türkçe
              </button>
              <button className="button is-small is-rounded" onClick={() => this.props.toggle('pl')}>
                Polski
              </button>
              <button className="button is-small is-rounded" onClick={() => this.props.toggle('ro')}>
                Română
              </button>
              <button className="button is-small is-rounded" onClick={() => this.props.toggle('fa')}>
                فارسی
              </button>
              <button className="button is-small is-rounded" onClick={() => this.props.toggle('uk')}>
                Українська
              </button>
              <button className="button is-small is-rounded" onClick={() => this.props.toggle('kr')}>
                한국어
              </button>
              <button className="button is-small is-rounded" onClick={() => this.props.toggle('af')}>
                Afrikaans
              </button>
