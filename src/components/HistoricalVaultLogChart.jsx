...
const HistoricalVaultLogChart = ({ ilksByName, vault, currentCollateralRatio, priceList }) => {
      ...
      logs = [
        {
          ...
          debtAfter:
            ilksByName && vault.collateralType
              ? parseFloat(vault.debt) * parseFloat(ilksByName[vault.collateralType.id].rate)
              : 0,
  ...
  const getLogsPercent = (priceList, logs, vault) => {
    ...
    return priceList.length && logs[0].timestamp - logs[logs.length - 1].timestamp > 60 * 60 * 24 * 7
