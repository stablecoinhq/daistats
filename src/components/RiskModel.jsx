...
function RiskModel(props) {
  ...
  const updateModel = () => {
    const run = async () => {
      ...
      let xAxisStart = 0, xAxisEnd = 0;
      if (maximumDebtCeilingValue > totalDebtByVaultTypeValue) {
        xAxisStart = Math.max(
          Math.min(totalDebtByVaultTypeValue, totalDebtByVaultTypeValue - (maximumDebtCeilingValue - totalDebtByVaultTypeValue) * 1),
          0
        )
        xAxisEnd = maximumDebtCeilingValue + (maximumDebtCeilingValue - totalDebtByVaultTypeValue) * 5
      } else {
        xAxisStart = Math.max(
          Math.min(maximumDebtCeilingValue, maximumDebtCeilingValue - (totalDebtByVaultTypeValue - maximumDebtCeilingValue) * 1),
          0
        )
