      ...
      <div>
        <div className="has-text-centered">
          <div className="box has-text-centered">
            ...
            <p className="subtitle is-size-4">{t("daistats.vaults_at_risk.min_collateral_ratio")}: {ilk.mat}</p>
            <p className="subtitle is-size-4">{t("daistats.vaults_at_risk.current_price")}: {ilk.price}</p>
            <p className="subtitle is-size-4">
            ...
            <p className="subtitle is-size-4">{t("daistats.vaults_at_risk.simulated_price")}: {ilk.price * priceDropRatio}</p>
          </div>
            ...
            <div className="columns" key={`vaultListIndex_${vaultListIndex}`}>
              {vaultList.map((vault) => (
                <div className="column box has-text-centered" key={vault.id}>
                  ...
                  <p className="subtitle is-size-6">{t("daistats.vaults_at_risk.collateral")}: {vault.collateral}</p>
                  <p className="subtitle is-size-6">{t("daistats.vaults_at_risk.debt")}: {parseFloat(vault.debt) * parseFloat(ilk.rate)}</p>
