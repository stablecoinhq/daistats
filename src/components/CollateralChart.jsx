...
const CollateralChart = ({ ilks, debt, useValue, groupBy }) => {
  ...
  const ilkPercent = (ilk) => {
    if (useValue) {
      return { 'name': ilk['ilk'], 'token': ilk['token'], 'value': (ilk.value / debt) * 100 };
    } else {
      return { 'name': ilk['ilk'], 'token': ilk['token'], 'value': ((ilk.Art * ilk.rate) / debt) * 100 };
    }
  };
  ...
  const reduce = (kv) => {
    return { 'name': kv[0], 'value': kv[1].reduce((t, v) => t + Number(v['value']), Number('0')) };
  };
  ...
  data.push({ 'name': 'Others', 'value': others.reduce((t, v) => t + v['value'], 0) });
        ...
        <PieChart>
          ...
          >
            {data.map((entry, index) => (
              <Cell fill={ILK_TO_COLOUR[entry.name]} key={index} />
            ))}
          </Pie>
